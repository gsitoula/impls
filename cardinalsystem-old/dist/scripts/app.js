    ...
    angular.module('app')
        .config(['$routeProvider', function($routeProvider) {
            ...
            routes = [
                ...
                'ventas/ventas', 'ventas/asignacion-campanas', 'ventas/asociar-subsets-campanas', 'ventas/campana-nueva', 'ventas/campanas', 'ventas/clientes', 'ventas/contratos', 'ventas/llamados', 'ventas/planes',
                'ventas/carga-campanas', 'ventas/campanas-asignadas', 'ventas/detalle-campanas-asignadas', 'ventas/campana-nueva',
                'ventas/carga-contratos', 'ventas/tabla-contratos', 'ventas/tabla-campanas', 
                ...
                'main/aplicaciones',
                'import/importacion',
                'settings/administracion', 'settings/sincronizacion-ad', 'settings/sincronizacion-tango'
            ]
...
(function () {
    ...
    angular.module('app')
        .controller('callcenterCtrl', [ '$scope', '$state', '$stateParams', '$http', callcenterCtrl]);

    function callcenterCtrl($scope, $state, $stateParams, $http) {
...
(function () {
  ...
  .config(function($stateProvider, $urlRouterProvider) {
    ...
    .state('nueva_asignacion_campana', {
      url: "ventas/asignacion-campanas",
      templateUrl: "app/ventas/asignacion-campanas.html"
    })
  ...
  .controller('campanasAsignadasCtrl', ['$scope', '$state', '$stateParams', '$http', 'NgTableParams', campanasAsignadasCtrl])
  .controller('detalleCampAsignadasCtrl', ['$scope', '$state', '$stateParams', '$http', 'NgTableParams', detalleCampAsignadasCtrl])
  .controller('contratosyCampanasCtrl', ['$scope', '$state', '$stateParams', '$http', 'NgTableParams', contratosyCampanasCtrl])
  .controller('cargaContratosCtrl', ['$scope', '$state', '$stateParams', '$http', 'NgTableParams', 'logger', cargaContratosCtrl])
  .controller('tablaCampanasCtrl', ['$scope', '$state', '$stateParams', '$http', 'NgTableParams', 'logger', tablaCampanasCtrl])
  ...
  function ventasCtrl($scope, $state, $stateParams, $http) {
    ...
    function campanasAsignadasCtrl($scope, $state, $stateParams, $http, NgTableParams) {
      //console.log("hola mundo");
      ...
      $scope.verDetalle = function(id) {
              //console.log(id);
              $state.go('detalle_campanas_asignadas', { id: id });
            };

          /*
          $scope.editFunction = function (id) {
            //console.log(id);
            $state.go('EditContrato', { id: id });
          };*/
        }

        function detalleCampAsignadasCtrl ($scope, $state, $stateParams, $http, NgTableParams, detalleCampAsignadasCtrl){
          console.log($stateParams.id);

          $scope.pushIt = function(user){
            console.log(user);
          };
          ...
          .then(function (data, status) {
            ...
            $scope.operadoresActuales = data.data[0].OPERADORES;

            $scope.form = {
              cliente: data.data[0].CLIENTE,
              campana: data.data[0].CAMPANA,
              puntodecontacto: data.data[0].PUNTO_DE_CONTACTO,
              fechainicio: data.data[0].FECHA_INICIO_OPE,
              fechafin: data.data[0].FECHA_FIN_OPE,
              comercial: data.data[0].RESPONSABLE_COMERCIAL,
              archivo: data.data[0].ARCHIVO_SELECCIONADO,
    ...
    $scope.volverCampanasAsignadas = function(){
      $state.go('campanas_asignadas');
    }
  }

  function contratosyCampanasCtrl($scope, $state, $stateParams, $http, NgTableParams) {
      //console.log("hola mundo");
    ...
    function cargaContratosCtrl($scope, $state, $stateParams, $http, NgTableParams, logger) {
      ...
      $scope.onSelectCliente = function(item) {
        //console.log(item);
        //$scope.id_cliente = item._id;
        //console.log(item);
        
        $scope.objCliente = {};
        $scope.objCliente.idDb = item._id;
        $scope.objCliente.nombre = item.NOM_COM;
      ...
      $scope.guardarContrato = function() {
        ...
        var datos_contrato = {
          fecha_inicio: $scope.form.fecha_inicio.toLocaleString().slice(0, $scope.form.fecha_inicio.toLocaleString().lastIndexOf(',')),
          fecha_fin: $scope.form.fecha_fin.toLocaleString().slice(0, $scope.form.fecha_fin.toLocaleString().lastIndexOf(',')),
          contrato_status: $scope.form.estado,
          contrato_scaneado: $scope.form.contrato_doc
        };   

        var tarifas_contrato = {
          tarifa_puesto_hora: $scope.form.puesto_hora,
          comision: $scope.form.comision,
          hs_presenciales: $scope.form.hs_presenciales,
          vph: $scope.form.vph,
          indice_mult: $scope.form.indice_mult
        };

        var objRegistro = {
          cliente: $scope.objCliente,
          responsable_comercial: $scope.objRespCom,
          datos_contrato: datos_contrato,
          tarifas_contrato: tarifas_contrato
        };

        //console.log(objRegistro);
        //$state.go('contratos_campanas');
      ...
      // Guarda el Contrato actual y envia al operador hacia la vista Carga Campañas
      $scope.asociarCampana = function() {
        ...
        .then(function (data, status) {
            //console.log(data.data.upserted[0]._id);
            //$state.go('contratos_campanas');
            $state.go('nueva_campana', { id: data.data.upserted[0]._id });                
          });

      };
    }

    function tablaCampanasCtrl($scope, $state, $stateParams, $http, NgTableParams, logger) {
    ...
    function cargaCampanasCtrl($scope, $state, $stateParams, $http, NgTableParams, logger) {
      ...
      $scope.onSelectCliente = function(item) {
        ...
        $scope.objCliente = {};
        $scope.objCliente.idDb = item._id;
        $scope.objCliente.nombre = item.nombre_real;
        
        $http({
            method: 'GET',
            url: config.url_service + '/getContrato',
            params: { collection: 'contratos', cliente_id: $scope.objCliente.idDb }
        })
            .then(function (data, status) {
                console.log(data.data[0]._id);
                $scope.ref_contrato = data.data[0]._id;
              })
            .catch(function(err) {
                console.log(err);
              });
      ...
      $scope.quitarRowRecurso = function() {
        //console.log($scope.recursos_array.lastIndexOf());
      ...
      $scope.asociarCampana = function() {
        //$state.go('nueva_campana');
        //console.log($scope.form.fecha_ini_campana);
        //console.log($scope.form.fecha_fin_campana);
        
        var objRegistro = {
          "CAMPANA": $scope.form.nombre_campana,
          "CLIENTE": $scope.objCliente,
          "CONTRATO": $scope.ref_contrato,
          "PUNTO_DE_CONTACTO": $scope.form.punto_contacto,
          "RESPONSABLE_COMERCIAL": $scope.form.resp_comercial,
...
(function () {
 ...
 angular.module('app')/*
    ...
    .controller('sincTangoCtrl', ['$scope', '$state', '$stateParams', '$http', 'NgTableParams', sincTangoCtrl]);
    ...
    function sincTangoCtrl($scope, $state, $stateParams, $http, NgTableParams) {
